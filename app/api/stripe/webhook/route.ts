...
import { getAccountProfile, hasStripeEventBeenProcessed, markStripeEventProcessed, saveAccountOrder, saveAccountSession } from '@/src/lib/account-store';
...
export async function POST(request: Request) {
  ...
  try {
    event = stripe.webhooks.constructEvent(payload, stripeSignature, stripeWebhookSecret);
  } catch (error) {
  ...
  if (!profile) {
    return NextResponse.json({ error: 'Account profile for checkout could not be found.' }, { status: 404 });
  }

  if (session.payment_status !== 'paid') {
    return NextResponse.json({ error: 'Checkout session is not paid yet.' }, { status: 402 });
  }
  ...
  return NextResponse.json({ received: true, fulfilled: true, orderId: order.id });
}
